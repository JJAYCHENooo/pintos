...
/* 定义系统调用的handler的类型，它接收32位长的参数地址，由这三个指针来提供 */
typedef int (*handler) (uint32_t, uint32_t, uint32_t);
static handler syscall_vec[128];/* 存放系统调用的handler数组，实现映射 */
...
void
syscall_init (void) //初始化系统调用，在main中被调用
{
  intr_register_int (0x30, 3, INTR_ON, syscall_handler, "syscall");
  /* 将系统调用处理函数 syscall_handler 注册为中断向量号（0 x30) 
   * 参数列表中的 3 代表用户可以调用这个中断 */
...
syscall_handler (struct intr_frame *f UNUSED) 
{
  ...
  p = f->esp;//获取这次系统调用的编号所在地址
  
  if (!is_user_vaddr (p))//不能访问内核地址
    goto terminate;
  
  if (*p < SYS_HALT || *p > SYS_INUMBER)//超出编号范围
    goto terminate;
  
  h = syscall_vec[*p];//取出系统调用函数
  
  //下一个参数必须在用户虚拟内存内
  if (!(is_user_vaddr (p + 1) && is_user_vaddr (p + 2) && is_user_vaddr (p + 3)))
    goto terminate;
  
  ret = h (*(p + 1), *(p + 2), *(p + 3));//执行相应的系统调用
...
sys_exit (int status)
{
  ...
  t = thread_current ();
  while (!list_empty (&t->files))//循环关闭所有用户打开的文件
    {
